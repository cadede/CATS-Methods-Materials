function [video, audio] = mmreadFT(filename, frames, time, disableVideo, disableAudio, matlabCommand, trySeeking, useFFGRAB)
...
% toc
        try 
            FFGrab('doCapture');  %this line takes the bulk of time, but don't know how else to read the frames if it doesn't do this line.
        catch
            err = lasterror;
...
% toc
                for f=1:nrFramesCaptured
                    [~, time] = FFGrab('getVideoFrame',i-1,f-1);
    ...
    try
        ...
        if strcmp(matlabCommand,'')
            ...
            for i=1:nrVideoStreams
                ...
                for f=1:nrFramesCaptured
                    [~, time] = mexDDGrab('getVideoFrame',i-1,f-1);
