function		rot_3d_model_NED(F,prh,speed)
...
% Modified 1/2018 -> still plots using NEU, but switches z-axis dot to show
% NED.
...
for k=1:size(prh,1),
    ...
    set(F.p,'Vertices',[tpts(:,1) -tpts(:,2) tpts(:,3)]) ;
	 ...
	 set(F.A(3),'xdata',-lz(2,1),'ydata',lz(2,2),'zdata',-lz(2,3)); % switch dot to right hand orientation 
	 if ~isempty(F.C),
		cx = F.CX*Q ;
