...
%This is based on vidsdatawireless, but it imports the raw VR files, plus a
%user-created "quad" file that is an unwrapped version of the VR video.
%More information available by contacting the authors.


tic;

firststart = 5*60+25; % enter starttime of the first video you want to render (in seconds).  0 if the whole video, something else if you want to cut some off
lastend = 6*60+30;% 78*60*60 + 3*60 + 43 - 100*60*60;%40; %enter a big number if you want to do the whole last video (anything longer than the length of the file- generally 100 hours in seconds to be bigger than the weird reading wireless videos)
...
gap = 4; %pixels between video and graph
...
%
wn = regexp(fileloc,'\'); try wn = fileloc(wn(end-2)+1:wn(end-1)-1); catch; wn = fileloc(wn(end-1)+1:wn(end)-1); end
quadloc = ['G:\Team Drives\Goldbogen Transfer\Permanent\' wn 'quads\'];
if ~exist(quadloc,'dir'); [~,quadloc] = uigetfile('*.*','select a file in quad file directory'); end
...
% if isempty(strfind(filename{1},'CATS')) && isempty(strfind(filename{1},'AW'))
...
[vidq,audq] = mmread([quadloc filename{1}(1:end-4) 'Quad.mp4'], [1 2]);
toc
% if abs(vidq.width/vidq.height-16/9)>.1; error ('Single video is not 16 x 9.  Back to the drawing board- more weekend matlabbing'); end
if vidq.width ~= 3840 || vidq.height~= 2160 || vid.width ~= 2880 || vid.height ~=2880; error ('New vid sizes, check all ratios'); end
%
vidW = vidq.width*1.5; vidH = vidq.height;
ysize = vidq.height/4*.8 - gap; % Half the size of one frame% round(2.15/14.3*vidH*14.3/10);%260;%215; % vertical pixel size of graph % assumes a screen area of a media player that is 14.3 cm tall by 30.8 wide
xsize = vidq.width; % size of two frames %round(21.5/17.7*vidW);
adF = 12; % legacy
...
if combinesmall; combos = getcombos(vidDN,vidDurs,viddeploy,1800,600); else combos = num2cell(viddeploy); end
...
for i = 1:length(viddeploy)
    ...
    text(DN2(a1),0,labs(i,:),'verticalalignment','bottom','fontsize',11+adF,'color','r')
end
...
set(fig5,'units','pixels','Position',[1,80,xsize,round(ysize*lowrat)]);%fix this
...
if CONTINUE; startn = n; else startn = 1; end % lastVideo = length of last video % lastVideo = 0;
...
%     for j = 1:3
        oj = j;
        if ci~=1; oj = j+1; end
        if j>stopj; break; end  %j*dur>endtime || dur*(j-1)+starttime(n) > endtime;
        clear vid vidq aud;
...
%         if j>1; ST0 = ST0-kitten*0.1; end
        ...
        [vidq,~] = mmread([quadloc filename{n}(1:end-4) 'Quad.mp4'], [],[ST0 min(dur+ST0,endtime)]); %e
...
%                 if j>=stopj; [ed,endf] = min(abs(oframeTimes{vidN}-vid.times(end))); 
%                     if ed>0.1;  error('vid.times(end) not fround in oframeTimes'); end
%                 else
...
%         end
        ...
        vidstampend = vidDN(combos{c}(end))+min([vidDurs(combos{c}(end)) et(n)])/24/60/60;
        %         sp1 = subplot(4,1,1:3);
        ...
        tagnan = double(tagon); tagnan(find(tagon,fs)) = 0; tagnan(~tagnan) = nan; 
        ...
        try usePaddles = INFO.usePaddles; end;
        ...
        ys = get(ax1(1),'ylim');
        if ~any(get(ax1(1),'ytick')>0 & get(ax1(1),'ytick')<max(get(ax1(1),'ylim'))); set(ax1(1),'ytick',[0 max(floor(max(ys)), 10*floor(max(ys-1)/10))]); end
        ...
        for i = 1:size(vNs,1)
            ...
            rectangle('position',[DN2(a1) ys(1) DN2(b1)-DN2(a1) ys(2)],'facecolor',fc/255);
            oi = get(gca,'children');
            set(gca,'children',[oi(2:end); oi(1)]);
            if i == 1; y2 = mean(ys); else y2 = 0; end
            vidts(i) = text(DN2(a1+20),y2,vNs(i,:),'verticalalignment','top','horizontalalignment','left','fontsize',10+adF,'color','r');
        end
...
%         leg = legend(hp6,'Gyros','location',legloc,'orientation','horizontal');
        ...
        set(fig3,'units','pixels','Position',[1,80,xsize,round(ysize*lowrat)]);%fix this
        set(gca,'units','points'); % these four lines repeated to ensure it stretches out right
        pos = get(gca,'position'); pos(1) = 80;
        set(gca,'position',pos); set(gca,'units','normalized'); pos = get(gca,'position');
        if vidW > 1500
...
%             end
            ...
            if i == 1 || DNclose>=DNclose0+.99/24/60/60/Hz% .99 is the wiggle room for rounding errors with doubles
                DNclose0 = DNclose;
                fig4 = figure(4); clf;
                set(fig4,'units','pixels','Position',[1,1,boxsize/2,boxH/2],'color','w','menubar','none');
%                 set(fig4,'Units','pixels', 'outerposition',[0 0 boxsize boxH], 'menubar','none');
%                 undecorateFig(fig4)
%                 set(gca,'Visible', 'Off', 'Position',[0 0 1 1],'xlim',[0 1920],'ylim',[0 1000]);
                ...
                Mbox = getframe(fig4);
                [t, tmap] = rgb2ind(Mbox.cdata,255);
                [t, tmap] = imresize(t,tmap,[boxH boxsize]);
                Mbox.cdata = ind2rgb(t,tmap);
                %
                ...
                if (vid.times(i)-startref(n)>3 && vid.times(i)-startref(n)<10 && ci == 1) || (ci == 1 && vid.times(end)<=3 && i>length(vid.times)/2)
                    ...
                    if length(combos{c}) == 1; fullt = text(x1,0,['Full Video ' num2str(vidN) ' Record'],'verticalalignment','top','horizontalalignment','left','fontsize',30,'fontweight','bold','parent',ax1(1));
                    else fullt = text(x1,0,['Full Video ' num2str(vidN) ' - ' num2str(combos{c}(end)) ' Record'],'verticalalignment','top','horizontalalignment','left','fontsize',28,'fontweight','bold','parent',ax1(1));
                    end
            ...
            oi = uint8(zeros(vidH+2*ysize + 2*gap,xsize*1.5,3)); %
            %             oi(1:vidH,1:vid.width,:) = vid.frames(i).cdata;
            [~,ii] = min(abs(vidq.times-vid.times(i))); % find the closest frame time to the original.  Should be the same except sometimes at the very beginning the quads have extra frames.
            
            oi(1:vidH,xsize/2+1:xsize*1.5,:) = vidq.frames(ii).cdata;
            oi(1:vidH/2,1:xsize/2,:) = oi(vidH/2+1:vidH,xsize+1:xsize*1.5,:);
            oi(vidH/2+1:vidH,xsize+1:xsize*1.5,:) = 0;
            ...
            oi(vidH+gap+1:vidH+gap+ysize,1:xsize,:) = M.cdata;
            oi(vidH+2*gap+1+ysize:end,1:xsize,:) = Mp.cdata;
            [t, tmap] = rgb2ind(vid.frames(i).cdata,255);
            [t, tmap] = imresize(t,tmap,[xsize/2-gap xsize/2-gap]);
            oi(vidH/2+gap+1:vidH/2+gap+size(t,1),xsize+gap+1:end,:) = ind2rgb(t,tmap)*255;

            oi(vidH/2+1:vidH,1:boxsize,:) = Mbox.cdata*255;
            t = oi(vidH/2+1:vidH,boxsize+1:xsize/2,:);
            tt = {'Left';'Forward';'Right';'Backward';'VR'};
            pos = [200 200; 400 200; 730 200; 380 400; 770 400]; pos(:,1) = pos(:,1)+50; pos(:,2) = pos(:,2)+200;
            tt = insertText(t,pos,tt,'fontsize',60,'textcolor','w');
            tt(tt<255) = 0;
%             figure; image(tt)
            oi(vidH/2+1:vidH,boxsize+1:xsize/2,:) = tt;
        ...
        vid.width = xsize*1.5;
        vid.height = size(oi,1);%ysize + 2*gap;
...
%         if ~exist('frameSize','var'); vid2 = mmread([fileloc filename{n}],[1 5]); frameSize = [vid2.width vid2.height]; end
        frameSize = [vidH+2*ysize + 2*gap,xsize*1.5];
        blankframe = vid.frames(end).cdata;
        blankframe(1:vidH/2,1:xsize*1.5,:) = 0;
        blankframe(vidH/2+1:vidH,2*boxsize+1:xsize*1.5,:) = 0;
        blankframe(vidH+1:end,xsize+1:end,:) = 0;
...
%% to fix the VR gaps that looked bad before.  It's fixed above now so this is unnecessary if the newest version is run
...
if ~comp; conf.videoCompressor = char(list(end)); else conf.videoCompressor = char(list(comp)); end
for i = 1:length(D)
    d = dir([fileloc '\' D(i).name]);
    ds = {d(:).name}';
    ds = ds(find(~cellfun(@isempty,cellfun(@(x) strfind(x,'gap'),ds,'uniformoutput',false)))-1);
    for ii = 1:length(ds)
        vid = mmread([fileloc '\' D(i).name '\' ds{ii}],[],[4.7 6]);
        if vid.totalDuration>6; error('last partial is longer than 5 seconds');
        elseif vid.totalDuration<4.7;
            vid = mmread([fileloc '\' D(i).name '\' ds{ii}],[],[vid.totalDuration-.1 vid.totalDuration+.1]);
        end
        %         frameSize = [vidH+2*ysize + 2*gap,xsize*1.5];
        blankframe = vid.frames(end).cdata;
        vidH = 2160; xsize = size(blankframe,2)*2/3;
        blankframe(1:vidH/2,1:xsize*1.5,:) = 0;
        boxH = vidH/2;
        boxsize = round(300/457*boxH);
        blankframe(vidH/2+1:vidH,boxsize+1:xsize*1.5,:) = 0;
        blankframe(vidH+1:end,xsize+1:end,:) = 0;
        for iii = 1:90; vid.frames(iii).cdata = blankframe; end
        vid.frames(91:end) = [];
        vid.times(1:90) = 1/30:1/30:3; vid.times(91:end) = [];
        vid.rate = 30;
        vid.totalDuration = 3;
%         tail = [tail 'gap'];
        mmwrite([fileloc '\' D(i).name '\' ds{ii}(1:end-4) 'gap.avi'],vid,conf);
    end
end
