function gtrack2kml (gtrack,tagon,fs,DN,kmlfs,startlat,startlong,UTC,whaleName,p, fileloc)
...
for ii = 1:length(lats)
    % To plot
    kmlstr = [kmlstr ge_point(longs(ii),lats(ii),-subP(ii),'timeSpanStart',datestr(tstart(ii)-UTC/24,'yyyy-mm-ddTHH:MM:SSZ'),'timeSpanStop',datestr(tstop(ii)-UTC/24,'yyyy-mm-ddTHH:MM:SSZ'))];
end
KML = ge_folder('Points',kmlstr);
outfile = [fileloc(1) ':\' whaleName '.kml'];
try 
    ge_output(outfile,KML); 
catch; d = regexp(fileloc,'\'); 
    outfile = [fileloc(1:d(3)) whaleName '.kml']; 
    ge_output(outfile,KML); 
end
